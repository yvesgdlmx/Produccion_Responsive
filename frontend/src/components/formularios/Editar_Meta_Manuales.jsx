...
const Editar_Meta_Manuales = ({ meta }) => {
  ...
  const editarMeta = async () => {
    try {
      ...
      const { data } = await clienteAxios.put(`/metas/metas-manuales/editar/${meta.id}`, metaActualizada);
      mostrarAlerta({
        msg: 'Meta actualizada correctamente',
        error: false
      });
      setTimeout(() => {
        navigate('/manuales_metas');
      }, 1200);
    } catch (error) {
      console.error(error);
      mostrarAlerta({
...
export default Editar_Meta_Manuales;