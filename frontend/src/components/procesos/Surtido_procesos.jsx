...
const Surtido_procesos = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Filtra los registros que contengan "LENS LOG" en el name
      const metasLensLog = responseMetas.data.registros.filter(registro =>
        registro.name.includes('LENS LOG')
      );
      // Ahora sumamos las metas separadamente por turno
      ...
      // Esta lógica determina los intervalos de cada turno en función de la hora actual
      if (ahora.hour() >= 22) {
      ...
      // Se filtran los registros para cada turno considerando la hora en México
      ...
      setTotalHits(total);
      // Cálculo de metas totales para cada turno aplicando las horas correspondientes
