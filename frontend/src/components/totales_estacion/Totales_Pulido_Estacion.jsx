...
const Totales_Pulido_Estacion = () => {
  ...
  // Efecto para el scroll si existe hash en la URL
  useEffect(() => {
  ...
  // Obtención y filtrado de datos según la jornada (del día anterior a las 22:00 hasta las 21:30 del día siguiente)
  useEffect(() => {
    const obtenerDatos = async () => {
      try {
        ...
        const ahora = moment();
        let inicioHoy = moment().startOf("day").add(22, "hours"); // 22:00
  ...
  // Función para agrupar los hits por hora utilizando todos los registros
  ...
  // Función que, dada una hora en formato "HH:mm" y el inicio de la jornada,
  // retorna el objeto moment que representa el inicio del bucket
  ...
  /* 
    ...
    - Si existe valor en hitsPorHora se retorna ese valor.
    - Si no, se obtiene el bucket (inicio y fin) y se comprueba si ya debió haber cerrado (con un margen de 5 minutos).
      En ese caso se retorna 0; de lo contrario, se retorna una cadena vacía para no mostrar la columna.
  */
  ...
  // Se arma un arreglo de columnas a partir de ordenTurnos. Solo se muestran aquellas que ya tienen valor (número o 0)
