...
const Totales_Produccion_Estacion = () => {
  ...
  useEffect(() => {
    const obtenerDatos = async () => {
      try {
        // Obtener meta de la meta manual para JOB COMPLETE
        const responseMetas = await clienteAxios("/metas/metas-manuales");
        const metaLensLog = responseMetas.data.registros.find(
  ...
  // Función para calcular el rango de cada bucket (cada bucket dura 1 hora)
  ...
  /*  
    ...
    - Si en hitsPorHora existe un valor para ese bucket, se retorna.
    - Si no existe, se revisa si el bucket ya debió haber cerrado (agregando un margen de 5 minutos).
      Si ya cerró, se retorna 0; de lo contrario se retorna "" para omitir la columna.
  */
  ...
  // Se construye un arreglo de columnas a partir del arreglo fijo de horas
  ...
  // Función para asignar una clase en base a si el valor cumple la meta
