...
const Totales_Generado_Estacion = () => {
  ...
  // Arreglo fijo con el orden de las horas (buckets)
  const ordenTurnos = [
  ...
  // Función para calcular el rango de horas de un bucket (1 hora).
  ...
  // Función para obtener datos, filtrar la jornada y calcular totales y metas
  useEffect(() => {
  ...
  /* 
    ...
    - Si existe un valor en el objeto hitsPorHora se retorna ese valor.
    - Si no existe, se determina si el bucket ya debió haberse cerrado (agregando un margen de 5 minutos).
      En ese caso se muestra 0; de lo contrario, se retorna una cadena vacía para no mostrar la columna.
  */
  const getDisplayValue = (horaStr) => {
    ...
    const margen = 5; // minutos de margen para esperar registros
    return ahora.isAfter(bucketFin.clone().add(margen, "minutes"))
      ? 0
      : "";
  };
  // Se arma un arreglo de columnas a partir del orden fijo y se filtran aquellas sin valor (cadena vacía)
