...
const Totales_Biselado_Estacion = () => {
  ...
  // Función para calcular el rango de cada bucket usando el helper formatearHora y sumándole 1 hora para obtener la hora final
  const calcularRangoHoras = (horaInicio) => {
    const horaInicioFormateada = formatearHora(horaInicio).slice(0, 5);
    const horaFin = new Date(new Date(`2000-01-01 ${horaInicio}`).getTime() + 60 * 60 * 1000)
      .toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" });
    return `${horaInicioFormateada} - ${horaFin}`;
  };
  // Función para obtener el instante de inicio de un bucket dado
  ...
  /*  
    Función que devuelve el valor a mostrar para cada bucket:
      - Si en hitsPorHora existe un valor para ese bucket, se retorna ese valor.
      - Si no existe, se verifica si el bucket ya debió haber cerrado (se suma un margen de 5 minutos);
        si ya cerró, se retorna 0; de lo contrario se retorna "" (cadena vacía) para no mostrar esa columna.
  */
  ...
  // Construir las columnas a partir del arreglo fijo de horas usando getDisplayValue
  ...
  // Función para asignar una clase en base a si se cumple la meta (se puede personalizar)
          ...
          <tbody>
            <tr className="font-semibold text-gray-700">
              ...
              {columnas.map((col, i) => (
                ...
                >
                  <span className={getClassName(col.valor, meta)}>{col.valor}</span>
                </td>
              ))}
            </tr>
          </tbody>
