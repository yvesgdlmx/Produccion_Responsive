...
const MetasManuales = () => {
  ...
  useEffect(() => {
    const obtenerMetas = async () => {
      try {
        const respuesta = await clienteAxios.get("/metas/metas-manuales");
  ...
  const handleInputChange = (e, id, campo) => {
    ...
    setMetas((prevMetas) =>
      prevMetas.map((meta) => (meta.id === id ? { ...meta, [campo]: value } : meta))
    );
  };
    ...
    try {
      const actualizaciones = metas.map(async (meta) => {
        ...
        const respuesta = await clienteAxios.put(
          `/metas/metas-manuales/editar/${id}`,
