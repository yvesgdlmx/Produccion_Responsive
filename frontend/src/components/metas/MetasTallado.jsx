...
const MetasTallado = () => {
  ...
  useEffect(() => {
    // Función para obtener las metas de tallado desde el endpoint
    const obtenerMetas = async () => {
      try {
        const respuesta = await clienteAxios.get("/metas/metas-tallados");
    ...
    try {
      const actualizaciones = metas.map(async (meta) => {
        ...
        const respuesta = await clienteAxios.put(
          `/metas/metas-tallados/editar/${id}`,
      ...
      console.error("Error al guardar las metas de tallado:", error);
