...
const MetasTerminados = () => {
  const [metas, setMetas] = useState([]);
  useEffect(() => {
    const obtenerMetas = async () => {
      try {
        const respuesta = await clienteAxios.get("/metas/metas-terminados");
        setMetas(respuesta.data.registros);
      } catch (error) {
        console.error("Error al obtener las metas terminados:", error);
      }
    };
    obtenerMetas();
  }, []);
    ...
    try {
      const actualizaciones = metas.map(async (meta) => {
        ...
        const respuesta = await clienteAxios.put(
          `/metas/metas-terminados/editar/${id}`,
      ...
      console.error("Error al guardar las metas terminados:", error);
      swal("Meta actualizada correctamente", "Se han guardado los cambios", "success");;
    }
  };
  return (
...
export default MetasTerminados;