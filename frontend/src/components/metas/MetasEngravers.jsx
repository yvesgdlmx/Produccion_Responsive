...
const MetasEngravers = () => {
  const [metas, setMetas] = useState([]);
  useEffect(() => {
    const obtenerMetas = async () => {
      try {
        const respuesta = await clienteAxios.get("/metas/metas-engravers");
    ...
    try {
      const actualizaciones = metas.map(async (meta) => {
        ...
        const respuesta = await clienteAxios.put(
          `/metas/metas-engravers/editar/${id}`,
      ...
      console.error("Error al guardar las metas de engravers:", error);
