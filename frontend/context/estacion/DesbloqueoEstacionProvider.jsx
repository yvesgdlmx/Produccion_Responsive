...
const DesbloqueoEstacionContext = createContext();
const DesbloqueoEstacionProvider = ({ children }) => {
  // Referencia para el scroll si la URL incluye "#desbloqueo"
  ...
  // Obtención de datos: metas y registros (hits)
  useEffect(() => {
    const obtenerDatos = async () => {
      try {
        ...
        // 3. Cargar las notas de hora y de turno para la sección "desbloqueo"
        cargarNotas();
        cargarNotasTurnos();
      } catch (error) {
        console.error("Error al obtener los datos en desbloqueo:", error);
      }
    };
    obtenerDatos();
  }, []);
  ...
  const calcularTotalesPorTurno = (registros, inicioJornada) => {
    ...
    registros.forEach((registro) => {
      ...
      // Turno Matutino: desde inicioJornada + 8h 30min hasta inicioJornada + 16h
      else if (
        fechaHoraRegistro.isBetween(
      ...
      // Turno Vespertino: desde inicioJornada + 16h 30min hasta inicioJornada + 23h 30min
      else if (
        fechaHoraRegistro.isBetween(
          inicioJornada.clone().add(16, "hours").add(30, "minutes"),
  ...
  // Armar el arreglo de columnas (buckets) a partir del arreglo de turnos
  ...
  // Función para retornar la meta por hora (según turno) para cada bucket
  ...
  // Función para asignar la clase CSS según se cumpla la meta
  ...
  const handleGuardarNota = async (hora) => {
    try {
      ...
      console.error("Error al guardar la nota en desbloqueo:", error);
    }
  };
  const handleEditarNota = async (hora) => {
    try {
      ...
      console.error("Error al editar la nota en desbloqueo:", error);
    }
  };
  const cargarNotas = async () => {
    try {
      ...
      console.error("Error al cargar las notas en desbloqueo:", error);
    }
  };
  ...
  const cargarNotasTurnos = async () => {
    try {
      ...
      console.error("Error al cargar las notas de turno en desbloqueo:", error);
    }
  };
  const handleGuardarNotaTurno = async (turno) => {
    try {
      ...
      console.error("Error al guardar la nota de turno en desbloqueo:", error);
    }
  };
  const handleEditarNotaTurno = async (turno) => {
    try {
      ...
      console.error("Error al editar la nota de turno en desbloqueo:", error);
    }
  };
  return (
    <DesbloqueoEstacionContext.Provider
      value={{
        desbloqueoRef,
        columnas,
        totalesPorTurno,
...
export { DesbloqueoEstacionProvider };
export default DesbloqueoEstacionContext;